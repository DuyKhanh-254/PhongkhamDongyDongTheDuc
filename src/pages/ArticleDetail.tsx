  ...
  if (!article) {
    return (
      <div className="section-padding text-center">
        <p className="text-muted-foreground">{String(t('sections.notFound')) || "Bài viết không tồn tại."}</p>
      ...
      <article className="section-padding bg-card">
        <div className="section-container max-w-3xl mx-auto">
          <Link to="/tin-tuc-kien-thuc" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary mb-6">
            <ArrowLeft className="w-4 h-4" /> {String(t('sections.back')) || "Quay lại"}
          </Link>
          ...
          <p className="text-sm text-muted-foreground mb-8">
            {new Date(article.date).toLocaleDateString(lang === 'ko' ? 'ko-KR' : lang === 'zh' ? 'zh-CN' : 'vi-VN', { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
          </p>
      ...
      {related.length > 0 && (
        <section className="section-padding" style={{ background: "linear-gradient(180deg, hsl(145,60%,97%) 0%, hsl(0,0%,100%) 100%)" }}>
          <div className="section-container">
            <h2 className="font-heading font-bold text-2xl text-foreground mb-8">{String(t('sections.relatedArticles')) || "Bài viết liên quan"}</h2>
